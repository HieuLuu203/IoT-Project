...
const countAction = require('../controllers/countAction.controller');
const countTempMore = require('../controllers/countTempMore.controller');
/**
 * @swagger
 * tags:
 *   - name: Fan
 *     description: Các endpoints API để quản lý trạng thái của quạt
 *   - name: Air
 *     description: Các endpoints API để quản lý trạng thái của điều hòa
 *   - name: Lamp
 *     description: Các endpoints API để quản lý trạng thái của đèn
 *   - name: Sensor
 *     description: Các endpoints API để truy xuất dữ liệu cảm biến
 */
 ...
 *     tags:
 *       - Device
 *     summary: Count device actions today
 *     description: Counts the number of times a device has been turned on or off today.
 *     parameters:
 *       - in: query
 *         name: deviceName
 *         schema:
 ...
 *     responses:
 *       200:
 *         description: Count of on/off actions
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: integer
 *                   example: 5
 *                   description: The number of times the device was turned on today.
 *                 countOff:
 *                   type: integer
 *                   example: 3
 *                   description: The number of times the device was turned off today.
 *       500:
 *         description: Internal server error
 *         content:
 *           application/json:
 ...
 */
router.get('/api/v1/count-action', countAction);

router.get('/api/v1/count-temp-more', countTempMore);
